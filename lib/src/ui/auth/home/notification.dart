...
class _NotificationScreenState extends State<NotificationScreen> {
  ...
  void _getNotificationListData({bool isLoading = true}) {
    ...
    UserApiManager().getNotificationList(page: _pageCount).then((value) {
  ...
  Widget slideAbleNotificationCell(
          {required int index, required NotificationListData data}) =>
      Padding(
        padding: const EdgeInsets.only(
            left: spacingLarge, right: spacingLarge, bottom: spacingMedium),
        child: Slidable(
      ...
      InkWell(
        onTap: () {
          if ((data.isRead ?? 0) == 0) {
            ...
            ProgressDialogUtils.showProgressDialog(context);
            UserApiManager().readNotification(id: notificationId).then((value) {
              ProgressDialogUtils.dismissProgressDialog();
              setState(() {
                data.isRead = 1;
              });
  ...
  void _goToNextScreen(NotificationListData data) {
    NavigationUtils.push(context, routeSupportTicketsChat,
        arguments: {NavigationParams.senderUserId: data.ticketId ?? 0});
  }
}
